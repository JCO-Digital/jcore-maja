...
The templates are organized into a folder structure to make them easier to find. Generally, the root folder contains full templates, and the sub-folders contain partial templates.

### Root views Folder

Contains full templates that are called directly by Timber and define the entire page. There are two notable exceptions to the full-page rule: the header.twig and footer.twig templates are used by the get_header() and get_footer() functions to preserve maximum compatibility with plugins.
...
- topbar.twig - Top menu that defaults to the secondary menu.
...
Contains WooCommerce templates. This folder is a bit of an exception to the rule in the sense that the root woo folder contains full templates and has its own structure of sub-folders.
